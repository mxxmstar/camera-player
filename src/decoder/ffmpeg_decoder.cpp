...
bool FFmpegDecoder::Open(const StreamInfo& info) {
    ...
    if (!decoder) {
        LOG_ERROR("FFmpegDecoder:Open: avcodec_find_decoder failed for id={}",
    ...
    if (!info.extra_data.empty()) {
        ...
        LOG_ERROR("FFmpegDecoder:Open: avmalloc for extradata failed");
    ...
    stream_info_ = info;
    LOG_INFO("FFmpegDecoder:Open success: codec={}, {}x{}",
             static_cast<int>(info.codec_type),

...
struct CaterAvtpPlayer::Runtime {
    Runtime()
...
bool CaterAvtpPlayer::StartOnIo(const std::string& device_name,
                                const std::string& source_mac,
                                const std::string& stream_id) {
    StopOnIo();
    ...
    // Use format=custom to force Cater CAM (format_subtype=0x00) parsing
    std::string url = "avtp://" + device + "?queue=1024&read_timeout=100&fps=25&format=custom";
    if (!source.empty()) {
        url += "&src=" + source;
    }
    if (!stream.empty()) {
    ...
    PublishState("监听中，等待卡特摄像头 AVTP/CVF");
    return true;
#else
    (void)device_name;
    (void)source_mac;
    (void)stream_id;
    PublishError("当前构建未启用 pcap，无法监听 Cater AVTP");
    return false;
#endif
}

void CaterAvtpPlayer::StopOnIo() {
...
void CaterAvtpPlayer::HandlePacket(std::shared_ptr<MediaPacket> packet) {
    if (!running_ || !packet) {
        return;
    }
...
void CaterAvtpPlayer::ReportStats() {
#ifdef ENABLE_PCAP
    ...
    LOG_INFO(
        "Cater AVTP RX: raw={}, video={}, filtered={}, parse_errors={}, "
        "cater_packets={}, cater_access_units={}, "
        "lost={}, malformed={}, access_units={}",
        stats.raw_packets,
        stats.parsed_video_packets,
        stats.filtered_packets,
        stats.parse_errors,
        stats.cater_cam_packets,
        stats.cater_cam_access_units,
        stats.assembler.lost_packets,
        stats.assembler.malformed_packets,
        stats.access_units);

    std::ostringstream message;
    message << "Cater AVTP 包=" << stats.raw_packets
            << ", 卡特帧=" << stats.cater_cam_access_units
...
std::vector<CaterAvtpPlayer::CaptureDevice> CaterAvtpPlayer::ListCaptureDevices() {
...
void CaterAvtpPlayer::ConvertAndPublishFrame(std::shared_ptr<MediaFrame> frame) {
    if (!running_ || !frame ||
...
void CaterAvtpPlayer::PublishFrame(std::shared_ptr<const MediaFrame> frame) {
...
void CaterAvtpPlayer::PublishState(const std::string& state) {
...
void CaterAvtpPlayer::PublishError(const std::string& error) {
    MessageCallback callback;
    {
        std::lock_guard<std::mutex> lock(callback_mutex_);

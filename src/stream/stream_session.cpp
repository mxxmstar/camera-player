...
StreamSession::StreamSession(asio::io_context& io)
    : io_(io)
    , reconnect_timer_(io)
    , watchdog_timer_(io)
    , decoder_timer_(io)
...
void StreamSession::OnDecoderDriveTimer(const asio::error_code& ec) {
...
void StreamSession::DoReconnect() {
    ...
    LOG_INFO("Reconnect attempt {} in {} ms",
...
void StreamSession::OnWatchdog(const asio::error_code& ec) {
    if (ec || !running_)
        return;
    ...
    if (idle_ms > watchdog_interval_ms_) {
        LOG_WARN("Watchdog timeout: idle={}ms > interval={}ms",

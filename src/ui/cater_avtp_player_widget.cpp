#include "ui/cater_avtp_player_widget.h"
...
CaterAvtpPlayerWidget::CaterAvtpPlayerWidget(QWidget *parent)
    : QWidget(parent)
{
    setupUi();
    setupConnections();
...
CaterAvtpPlayerWidget::~CaterAvtpPlayerWidget()
{
    if (m_avtpPlayer)
    {
        m_avtpPlayer->Stop();
...
void CaterAvtpPlayerWidget::setupUi()
{
    ...
    m_sourceMacEdit->setPlaceholderText(tr("e.g. 00:0a:75:60:3f:aa"));
    m_sourceMacEdit->setToolTip(tr("Only accept AVTP packets from this source MAC."));

    m_streamIdEdit = new QLineEdit(this);
    m_streamIdEdit->setPlaceholderText(tr("optional, e.g. 0x0123456789abcdef"));
    ...
    m_videoLabel->setText(tr("Start Cater AVTP listening before sending the "
    ...
    m_avtpPlayer = std::make_unique<CaterAvtpPlayer>();
    refreshCaptureDevices();
    updateControlState();
}

void CaterAvtpPlayerWidget::setupConnections()
{
    connect(m_refreshDevicesButton, &QPushButton::clicked, this, &CaterAvtpPlayerWidget::refreshCaptureDevices);
    connect(m_captureDeviceCombo, QOverload<int>::of(&QComboBox::currentIndexChanged), this, [this](int index)
    {
        Q_UNUSED(index);
        captureBroadcastMac();
    });
    connect(m_startButton, &QPushButton::clicked, this, &CaterAvtpPlayerWidget::startCaterAvtpPlayer);
    connect(m_stopButton, &QPushButton::clicked, this, &CaterAvtpPlayerWidget::stopCaterAvtpPlayer);
...
void CaterAvtpPlayerWidget::refreshCaptureDevices()
{
    ...
    const auto devices = CaterAvtpPlayer::ListCaptureDevices();
...
void CaterAvtpPlayerWidget::captureBroadcastMac()
{
...
void CaterAvtpPlayerWidget::updateControlState()
{
...
void CaterAvtpPlayerWidget::startCaterAvtpPlayer()
{
...
void CaterAvtpPlayerWidget::stopCaterAvtpPlayer()
{
    m_avtpPlayer->Stop();
    updateControlState();
}

void CaterAvtpPlayerWidget::onVideoFrame(std::shared_ptr<const MediaFrame> frame)
{
...
void CaterAvtpPlayerWidget::updateVideoPixmap()
{
...
void CaterAvtpPlayerWidget::resizeEvent(QResizeEvent *event)
{
